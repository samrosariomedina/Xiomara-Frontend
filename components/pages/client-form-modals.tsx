...
export function ClientFormModal({ isOpen, onClose, onSubmit }: ClientFormModalProps) {
  ...
  // Track form validity for each tab
  const [isGeneralFormValid, setIsGeneralFormValid] = useState(false)
  const [isConnectFormValid, setIsConnectFormValid] = useState(false)
  const [isBrandFormValid, setIsBrandFormValid] = useState(true) // Default to true since this form is minimal
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="flex items-center justify-end gap-3 px-6 py-4 border-t bg-white flex-shrink-0">
            <Button
              variant="outline"
              onClick={handleClose}
              className="text-[#31499F] border-gray-300 hover:bg-[#31499F] hover:text-[#f7f9ff] bg-[#F7F9FF] text-sm h-9 w-38 px-6 font-medium rounded-full"
              type="button"
            >
              {t('actions.cancel')}
            </Button>
            <Button
              onClick={() => {
                // Make the button a dummy (no-op) when the Connect tab is active
                if (activeTab === "connect") return
                void handleFormSubmit()
              }}
              disabled={isSubmitting || (activeTab === "general" ? !isGeneralFormValid : 
                        activeTab === "connect" ? !isConnectFormValid : 
                        !isBrandFormValid)}
              className={`text-[#f7f9ff] text-sm h-9 w-38 px-6 font-medium bg-[#31499F] hover:bg-[#f7f9ff] hover:text-[#31499f] rounded-full flex items-center justify-center gap-2`}
            >
              {isSubmitting ? (
                <>
